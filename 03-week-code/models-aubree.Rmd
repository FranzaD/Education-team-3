...
## account for media wave 4. This model will have to be updated
...
model1 <- lm(total_score_w4 ~ daily_use + total_score_w1, data = model1_data)
summary(model1)
```
...
```{r}
##
model2_data <- inner_join(pls_w6, pls_w1, by = c("child_id"), suffix = c("_w6", "_w1"))



model2_data <- inner_join(model2_data, media_data, by = c("child_id"))

model2 <- lm(total_score_w6 ~ daily_use_w1 + total_score_w1, data = media_pls_w1_w6)
summary(model2)
```
