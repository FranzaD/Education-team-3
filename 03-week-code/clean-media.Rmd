---
title: "clean media"
author: "James"
date: '2022-07-12'
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
library(forcats)
library(haven)
library(readxl)
library(ggplot2)
library(tidyverse)
library(dplyr)
library(janitor)

media_w1 <- readRDS(file="../data/media_data.dta") # which contain cleaned media_w1
media_w6 <- readRDS(file = "../data/media_wave_6.dta")
```

# create all media in wide format

```{r}
media_wide <- inner_join(mutate(media_w1),
           mutate(media_w6),
           by = c("child_id", "par_id"),
           suffix = c(".1", ".6")) %>%
        select(matches("_id"), matches("daily_use"))
```

# create all media in long format

```{r}
media_long <- reshape(as.data.frame(media_wide),
  varying = c("n_devices.1", "n_devices.6", "tv_exposure.1", "tv_exposure.6",
              "solo.1", "solo.6", "passive.1", "passive.6", "coop.1", "coop.6",
              "mealtimes.1", "mealtimes.6", "behavior_management.1", 
              "behavior_management.6", "positive_beliefs_media.1",
              "positive_beliefs_media.6", "educational_benefits.1",
              "educational_benefits.6", "digital_babysitting.1",
              "digital_babysitting.6", "daily_use.1", "daily_use.6"), 
  v.names = c("n_devices", "tv_exposure", "solo", "passive", "coop", "mealtimes",
              "behavior_management", "positive_beliefs_media", 
              "educational_benefits", "digital_babysitting", "daily_use"),
  timevar = "wave",
  times = c("1","6"),
  direction = "long")
```

# save data
```{r}
saveRDS(media_long, file="../data/media_long.dta")
saveRDS(media_wide, file="../data/media_wide.dta")
```
# load data back
```{r}
media_long <- readRDS(file="../data/media_long.dta")
media_wide <- readRDS(file="../data/media_wide.dta")
```