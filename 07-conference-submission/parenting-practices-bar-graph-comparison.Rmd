...
media_w1 %>% 
  ...
  na.omit() %>% 
  group_by(parent) %>% 
  summarize(solo = mean(solo),
            coop = mean(coop),
            tv_exposure = mean(tv_exposure),
            passive = mean(passive),
            mealtimes = mean(mealtimes),
            behavior_management = mean(behavior_management),
            digital_babysitting = mean(digital_babysitting),
            daily_use = mean(daily_use)) %>% 
