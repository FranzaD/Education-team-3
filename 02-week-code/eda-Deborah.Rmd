...
#now there are missing value codes and NAs I need to get rid of:
...
```{r}
# ideally only keep observations that completed the study
# pls & media, wave 1 & wave 6

#media wave6 + media wave6 + pls wave 1
anova_table <- full_join(valuable_dataset,model_w6, key = c(child_id, parent_id))

anova_table <- anova_table %>% 
  select( par_id, child_id, wave, wave.y.y, daily_use, total_score)

#media wave6 + media wave6 + pls wave 1 + pls wave 6
anova_table <- full_join(anova_table, PLS_data, key = c(child_id, parent_id))

anova_table <- anova_table %>% 
  filter(wave == 6 | wave == 1) %>% 
  select(par_id, child_id, wave, daily_use, total_score) %>% 
  na.omit()

#selecting what variables I want
#anova_table %>% 
#  select(total_score, daily_use) %>% 
#  filter( wave == 6 & wave == 1)
```
```{r}
unique(anova_table$wave)
```
