...
media_pls <- media %>%
  ...
  full_join(pls, by = c("par_id", "child_id", "wave")) %>%
  inner_join(grouping, by = c("child_id")) %>%
  inner_join(select(demo, child_id, par_id, father), by = c("child_id", "par_id"))
...
summary(model <- lmer(plspr ~ behavior_management + + (1 | child_id), REML = FALSE, data = media_pls_mother))
...
model2 <- lmer(plspr ~ behavior_management + wave + behavior_management:wave + (1 | child_id), REML = FALSE, data = media_pls_mother)
