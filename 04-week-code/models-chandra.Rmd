...
model0 <- lmer(plspr ~ behavior_management:father:wave + treatment + (1 | child_id), REML = FALSE, data = media_pls)
...
##  potential covariate explorations
### par_ethnicity
model <- lmer(plspr ~ behavior_management:father:wave + condition + par_ethnicity + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### enough_money_medical_care
model <- lmer(plspr ~ behavior_management:father:wave + condition + enough_money_medical_care + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### ssi_ssdi
model <- lmer(plspr ~ behavior_management:father:wave + condition + ssi_and_ssdi + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### planned_preg
model <- lmer(plspr ~ behavior_management:father:wave + condition + planned_preg + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### parenting_class_taken
model <- lmer(plspr ~ behavior_management:father:wave + condition + parenting_class_taken + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### data_site
model <- lmer(plspr ~ behavior_management:father:wave + condition + data_site + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### state_welfare
model <- lmer(plspr ~ behavior_management:father:wave + condition + state_welfare + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### span_lang_form
model <- lmer(plspr ~ behavior_management:father:wave + condition + span_lang_form + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### receive_childcare
model <- lmer(plspr ~ behavior_management:father:wave + condition + recieve_childcare + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### highest_degree_completed
model <- lmer(plspr ~ behavior_management:father:wave + condition + highest_degree_completed + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### years_school_complete -- condition became significant
model <- lmer(plspr ~ behavior_management:father:wave + condition + years_school_complete + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### adding all covariates as suggested by Dr. Reich's email
model <- lmer(plspr ~ behavior_management:father:wave + condition + span_lang_form + highest_degree_completed + years_school_complete + parenting_class_taken + recieve_childcare + assistance + (1 | child_id), REML = FALSE, data = media_pls)
summary(model)

### Jame's suggestion
model <- lmer(plspr ~ behavior_management:wave + condition + (1 | child_id), REML = FALSE, data = media_pls)

summary(model)

# Dr Reich's email

## Dr Reich's suggestion #1
model <- lmer(plspr ~ behavior_management:father:wave + treatment + (1 | child_id), REML = FALSE, data = media_pls)

summary(model)

## Dr Reich's suggestion #2
model <- lmer(plspr ~ behavior_management:father:wave + control + (1 | child_id), REML = FALSE, data = media_pls)

summary(model)

## Dr Reich's suggestion #3
model <- lmer(plspr ~ behavior_management + behavior_management:father:wave + behavior_management:treatment + (1 | child_id), REML = FALSE, data = media_pls)

summary(model)

## Dr Reich's request #4
### behavior management
bm_summary_waves <- media_pls %>% 
  group_by(wave) %>% 
  summarize(mean = mean(behavior_management, na.rm = TRUE), 
            min = min(behavior_management, na.rm = TRUE), 
            max = max(behavior_management, na.rm = TRUE), 
            sd = sd(behavior_management, na.rm = TRUE))

bm_treatment_summary <- media_pls %>%
  group_by(treatment) %>%
  summarize(mean = mean(behavior_management, na.rm = TRUE), 
            min = min(behavior_management, na.rm = TRUE), 
            max = max(behavior_management, na.rm = TRUE), 
...
### plspr
plspr_summary_waves <- media_pls %>% 
  group_by(wave) %>% 
  summarize(mean = mean(plspr, na.rm = TRUE), 
            min = min(plspr, na.rm = TRUE), 
            max = max(plspr, na.rm = TRUE), 
            sd = sd(plspr, na.rm = TRUE))

plspr_treatment_summary <- media_pls %>%
  group_by(treatment) %>%
  summarize(mean = mean(plspr, na.rm = TRUE), 
            min = min(plspr, na.rm = TRUE), 
            max = max(plspr, na.rm = TRUE), 
...
### behavior management by group for each wave
bm_group_summary_w1 <- media_pls %>%
  filter(wave == 1) %>% 
  group_by(treatment) %>%
  summarize(mean_bm_w1 = mean(behavior_management, na.rm = TRUE), 
            min_bm_w1 = min(behavior_management, na.rm = TRUE), 
            max_bm_w1 = max(behavior_management, na.rm = TRUE), 
            sd_bm_w1 = sd(behavior_management, na.rm = TRUE))

bm_group_summary_w4 <- media_pls %>%
  filter(wave == 4) %>% 
  group_by(treatment) %>%
  summarize(mean_bm_w4 = mean(behavior_management, na.rm = TRUE), 
            min_bm_w4= min(behavior_management, na.rm = TRUE), 
            max_bm_w4 = max(behavior_management, na.rm = TRUE), 
            sd_bm_w4 = sd(behavior_management, na.rm = TRUE))

bm_group_summary_w6 <- media_pls %>%
  filter(wave == 6) %>% 
  group_by(treatment) %>%
  summarize(mean_bm_w6 = mean(behavior_management, na.rm = TRUE), 
            min_bm_w6 = min(behavior_management, na.rm = TRUE), 
            max_bm_w6 = max(behavior_management, na.rm = TRUE), 
            sd_bm_w6 = sd(behavior_management, na.rm = TRUE))

### PLS percentil rank by group for each wave
plspr_group_summary_w1 <- media_pls %>%
  filter(wave == 1) %>% 
  group_by(treatment) %>%
  summarize(mean_plspr_w1 = mean(plspr, na.rm = TRUE), 
            min_plspr_w1 = min(plspr, na.rm = TRUE), 
            max_plspr_w1 = max(plspr, na.rm = TRUE), 
            sd_plspr_w1 = sd(plspr, na.rm = TRUE))

plspr_group_summary_w4 <- media_pls %>%
  filter(wave == 4) %>% 
  group_by(treatment) %>%
  summarize(mean_plspr_w4 = mean(plspr, na.rm = TRUE), 
            min_plspr_w4 = min(plspr, na.rm = TRUE), 
            max_plspr_w4 = max(plspr, na.rm = TRUE), 
            sd_plspr_w4 = sd(plspr, na.rm = TRUE))

plspr_group_summary_w6 <- media_pls %>%
  filter(wave == 6) %>% 
  group_by(treatment) %>%
  summarize(mean_plspr_w6 = mean(plspr, na.rm = TRUE), 
            min_plspr_w6 = min(plspr, na.rm = TRUE), 
            max_plspr_w6 = max(plspr, na.rm = TRUE), 
            sd_plspr_w6 = sd(plspr, na.rm = TRUE))
