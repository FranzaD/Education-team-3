...
parent_education <- parent_education %>%
  ...
  zap_labels(bg14) %>% # gets rid of default datatype
    mutate(par_id = as.character(parid),
         child_id  = as.character(childid),
         wave = as.factor(wave),
         parent_gender = as.logical(pargen),
...
parent_english_fluency <- parent_english_fluency %>% 
  zap_labels() %>% 
  mutate(par_id = as.character(parid),
         child_id  = as.character(childid),
         wave = as.factor(wave),
         parent_gender = as.logical(pargen),
         speaking_english = as.factor(bg25r),
         reading_english = as.factor(bg26r),
         writing_english = as.factor(bg27r),
         understanding_english = as.factor(bg28r),
         english_level = as.numeric(englishlanguage)
         ) %>% 
  select(par_id,
         child_id,
         wave,
