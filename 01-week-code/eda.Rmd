...
wave_6_codebook <- read_excel("../data/wave-6-(24-mo)/BB2_w6_BITSEA_codebook.xls.xlsx")
```
...
# join all demograph
```{r}
demographic_dataset = inner_join(media_tech_cleaned, parent_education, by=c("child_id", "par_id")) 
demographic_dataset = inner_join(demographic_dataset, parent_demographic, by=c("child_id", "par_id"))  
demographic_dataset = inner_join(demographic_dataset, public_assistance_w1, by=c("child_id", "par_id"))
demographic_dataset = inner_join(demographic_dataset, access_to_childcare_w1, by=c("child_id", "par_id")) 
demographic_dataset = inner_join(demographic_dataset, income_w1, by=c("child_id", "par_id")) 
demographic_dataset = inner_join(demographic_dataset, parent_english_fluency, 
                              by = c("child_id", "par_id"))
```
...
```{r}
# main dataset James'
saveRDS(model_w2, file="../data/BITSEA_wave_2.dta")
saveRDS(model_w6, file="../data/BITSEA_wave_6.dta")
saveRDS(model_score, file="../data/PLS_wave_1.dta")
saveRDS(demographic_dataset, file="../data/demographic_dataset.dta")
